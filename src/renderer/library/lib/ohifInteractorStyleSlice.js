...
function ohifInteractorStyleSlice (publicAPI, model) {
   ...
   */
  publicAPI.moveSliceByWheel = increment => {
    const slice = publicAPI.findSlice()
    const props = publicAPI.getDirectionalProperties()

    if (slice) {
      const renderer = model.interactor.getCurrentRenderer()
      renderer.getActiveCamera().setParallelProjection(true)

      const mode = slice.getMapper().getSlicingMode()
      let currentPosition
      let newPos
      let worldPos
      switch (mode) {
      ...
      const idx = slice.getMapper().getSliceAtPosition(newPos)
      ...
      if (publicAPI.getLastCameraPosition() != undefined) {
        const pos = publicAPI.getLastCameraPosition()
        renderer.getActiveCamera().setPosition(pos[0], pos[1], pos[2])
      } else {
        renderer.resetCamera()
      }

      const viewDirection = publicAPI.getViewDirection()
      const displaySet = publicAPI.getDisplaySet()
      const w = model.currentImageProperty.getColorWindow()
      const l = model.currentImageProperty.getColorLevel()
