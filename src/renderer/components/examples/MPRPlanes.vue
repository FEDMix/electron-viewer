<template>
  <div>
    ...
    <div v-else>
      <div>
        <button :class="{ active: activeTool === 'LEVEL' }" @click="selectTool('LEVEL')">
          <img src="https://img.icons8.com/material-rounded/344/contrast.png">
          Level
        </button>
        <button :class="{ active: activeTool === 'SELECT' }" @click="selectTool('SELECT')">
      ...
      <div class="row">
        <div v-for="(view, key) in viewDataArray" :key="key" class="col">
          <table style="background: white; margin: 10px; border-radius: 6px; padding: 4px;">
            ...
            <tr>
              <td>Rotate View</td>
              <td>
                <input :id="`${key}1rot`" v-model="view.viewRotation" type="radio" :value="0">
                <label :for="`${key}1rot`">0&deg;</label>

                <input :id="`${key}2rot`" v-model="view.viewRotation" type="radio" :value="90">
                <label :for="`${key}2rot`">90&deg;</label>

                <input :id="`${key}3rot`" v-model="view.viewRotation" type="radio" :value="180">
                <label :for="`${key}3rot`">180&deg;</label>

                <input :id="`${key}4rot`" v-model="view.viewRotation" type="radio" :value="270">
            ...
            <tr>
              <td>Blend Mode</td>
              <td>
                <input :id="`${key}1blend`" v-model="view.blendMode" type="radio" value="none">
                <label :for="`${key}1blend`">None</label>

                <input :id="`${key}2blend`" v-model="view.blendMode" type="radio" value="MIP">
                <label :for="`${key}2blend`">MIP</label>

                <input :id="`${key}3blend`" v-model="view.blendMode" type="radio" value="MINIP">
                <label :for="`${key}3blend`">MinIP</label>

                <input :id="`${key}4blend`" v-model="view.blendMode" type="radio" value="AVG">
...
const getPlaneIntersection = (plane1, plane2, plane3) => {
  try {
    const line = vtkPlane.intersectWithPlane(
      plane1.position,
      plane1.normal,
      plane2.position,
      plane2.normal
