<template>
  <div>
    ...
    <div v-else>
      <div>
        <table>
          ...
          <tr>
            <td>Slice Rotate</td>
            <td>
              <input v-model="rotate" class="rotate" type="range" :min="-90" :max="90" step="1">
      ...
      <div class="row">
        <div class="col">
          <view-2d :volumes="volumes" :on-created="this.saveComponentReference(2)" />
        </div>
        <div class="col">
          <view-generic :on-created="this.saveComponentReference(3)" :data-details="dataDetails" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  watch: {
    ...
    rotate (newRot) {
      const component = this.components[2]
